...
def get_challis_events(starttime, stoptime,minlatitude,maxlatitude, minlongitude, maxlongitude):
    ...
    Ch_Stat = client.get_stations(starttime=starttime, endtime=stoptime, minlatitude=minlatitude, maxlatitude =maxlatitude,
                            minlongitude=minlongitude, maxlongitude=maxlongitude, network = "*", station = "*", level= "response" )


    print(Ch_Stat)
    # CH_Stat_coord = Ch_Stat.
...
if __name__ == '__main__':
    ...
    if make_catalog_Sulphur:
        ...
        cat_s = read_events("Sulphur_catalog.xml")
        print(cat_s)
##
    if make_catalog_Challis:
        ...
        cat_c.write("Challis_stations.xml", format="stationxml")
    else:  # make_catalog=False
        import os
        from obspy.io.stationxml.core import _read_stationxml
        from obspy import read_inventory
        inv = read_inventory("./Stanley_EQT/Challis_stations.xml")
        stat= inv[0]
        print(stat)
        # StartDate 
        # EndDate = inv.Station.termination_date
        cat_c = _read_stationxml("./Stanley_EQT/Challis_stations.xml")
        print(cat_c)
